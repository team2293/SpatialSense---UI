          ...
          <div className="p-4 border-b border-zinc-700 flex-1 overflow-y-auto">
            ...
            <div className="space-y-2">
              {measurements.length === 0 && areaMeasurements.length === 0 ? (
                ...
                {areaMeasurements.map((a) => (
                  <div
                    key={a.id}
                    onClick={() => setSelectedMeasurement(selectedMeasurement === a.id ? null : a.id)}
                    className={`rounded p-3 cursor-pointer transition border group ${
                      selectedMeasurement === a.id
                        ? 'bg-yellow-500/20 border-yellow-500/50 ring-1 ring-yellow-500/30'
                        : 'bg-zinc-900 border-transparent hover:bg-zinc-700/50 hover:border-zinc-600'
                    }`}
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {renamingMeasurement === a.id ? (
                          <input
                            ...
                            onBlur={saveRename}
                            onClick={(e) => e.stopPropagation()}
                            className="font-mono text-xs text-yellow-400 bg-zinc-800 border border-yellow-500/50 rounded px-1 py-0.5 outline-none focus:ring-1 focus:ring-yellow-500/50 w-24"
                            autoFocus
                          />
                        ) : (
                          <span
                            onClick={(e) => handleNameClick(e, a.id, a.name)}
                            className={`font-mono text-xs cursor-text hover:underline ${selectedMeasurement === a.id ? 'text-yellow-400' : 'text-cyan-400'}`}
                            title="Click to rename"
                          >
                            {a.name}
                          </span>
                        )}
                        {selectedMeasurement === a.id && renamingMeasurement !== a.id && (
                          <span className="text-yellow-500 text-xs">(selected)</span>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-white font-mono font-medium">{formatArea(a.area)}</span>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setAreaMeasurements(prev => prev.filter(item => item.id !== a.id));
                            if (selectedMeasurement === a.id) setSelectedMeasurement(null);
                          }}
                          className={`w-5 h-5 rounded flex items-center justify-center transition ${
                            selectedMeasurement === a.id
                              ? 'text-red-400 hover:bg-red-500/30'
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-zinc-500 text-xs">Area · {a.vertices.length} vertices</span>
                    </div>
                  </div>
                ))}
                </>
